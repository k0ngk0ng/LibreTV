...
function initPlayer(videoUrl) {
    ...
    art.on('video:loadedmetadata', async function() {
        document.getElementById('player-loading').style.display = 'none';
        ...
        if (activeMediaContext !== loadContext || playbackLoadGeneration !== loadContext.generation) {
            return;
        }
...
function captureActivePlaybackSnapshot() {
    ...
    return {
        ...
        playbackPosition: Number(art.video.currentTime) || 0,
        duration: Number(art.video.duration) || 0,
    };
}
...
async function saveToHistory({ includeEpisodes = true, keepalive = false, playbackSnapshot = null } = {}) {
    ...
    try {
        await saveViewingHistory(videoInfo, { keepalive });
    } catch (error) {
        console.warn('保存服务器观看进度失败:', error);
    }
}
...
function saveCurrentProgress(event) {
    ...
    // art.switch 会暂停旧视频；此时目标集已更新，但旧媒体尚未完成切换。
    // playEpisode 已显式保存旧集进度，因此忽略这次切换产生的 pause。
    if (event?.type === 'pause' && snapshot.generation !== playbackLoadGeneration) return;

    saveToHistory({
        includeEpisodes: false,
        keepalive: event?.type === 'beforeunload' || event?.type === 'pagehide',
