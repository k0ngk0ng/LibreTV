...
- Develop customized features on `main`.
- `legacy` mirrors the upstream project and must not overwrite `main` without a reviewed migration.
...
- `config/sites.json`: bundled video-source configuration. Do not hard-code built-in sources back into frontend JavaScript.
...
- `js/player.js`: playback, episodes, progress, and source switching.
...
- `.github/workflows/sync.yml`: upstream synchronization to `legacy` only.

Never commit `.env`, `data/`, `.build-version`, cookies, CSRF tokens, real passwords, or production account/history data.
...
6. the page entry point, such as `js/app.js` or `js/player.js`

`player.html` does not load `app.js`. Therefore `player.js` must not depend on symbols defined only in `app.js`. Shared behavior belongs in a shared script and player startup must degrade safely if a nonessential shared helper is unavailable.
...
Formal releases use strict `vX.Y.Z` tags and should match `package.json`. A release tag publishes that tag plus `latest`; a `main` push publishes the 12-character commit tag.

## Runtime video-source configuration

Built-in sources are loaded only from the JSON file specified by `API_CONFIG_FILE`. Relative paths resolve from the application root. Docker deployments normally use an absolute container path and a read-only bind mount.
...
Docker images keep an unmounted fallback copy of the bundled source list. Invalid custom configuration falls back by default so a mount mistake does not take down the site. `API_CONFIG_STRICT=true` intentionally changes this to fail-fast behavior.
...
Verify the bundled file, an absolute custom path, Docker read-only mounting, fallback behavior, strict failure behavior, invalid JSON/URL/name rejection, and updated configuration after restart.
